# ALB Module for 2048 CI/CD Enterprise
# Creates Application Load Balancer with Blue-Green deployment support
...
# Target Group - Green (Staging/Testing)
...
# Listener Rule for Test Traffic (Green environment)
# This allows testing the green environment before full cutover
resource "aws_lb_listener_rule" "test_traffic" {
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "${var.project_name}-${var.environment}-test-traffic-rule"
    }
  )
}
